...
def log(msg, flag=None):
    ...
    if not os.path.isfile("./debug.log"):
        assert subprocess.call(f"echo \"[{now}_{head[flag]}] > {msg}\" > debug.log", shell=True)==0, print(f"[error] > shell command failed to execute")
    else: assert subprocess.call(f"echo \"[{now}_{head[flag]}] > {msg}\" >> debug.log", shell=True)==0, print(f"[error] > shell command failed to execute")
